...
$sql = 'BEGIN TAX_COST_SP(:state, :subtot, :tax); END;';
 
 $stmt = oci_parse($conn,$sql);
 
 // Bind the input parameter
 oci_bind_by_name($stmt,':state',$state,32);
 
 // Bind the output parameter
 oci_bind_by_name($stmt,':subtot',$subtot,32);
 oci_bind_by_name($stmt,':tax',$tax,32);
...
oci_close($conn);
 ?>
<br>
<a href="taxcal.php">Back to submition page</a>     
